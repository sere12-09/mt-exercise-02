...
# my line here below that I just added for the "logs"
...
# Writing the data and splitting it in the "split epoch perplexity" sections inside the "logs" folder:
if args.log_file != '':
    with open(args.log_file, "w") as f:
        f.write("split\tepoch\tperplexity\n")

def write_log(split, epoch, perplexity):
    if args.log_file == '':
        return

    with open(args.log_file, "a") as f:
        f.write(f"{split}\t{epoch}\t{perplexity}\n")
...
def train():
    ...
    # only RNN/LSTM/GRU need a hidden state
    if args.model != 'Transformer':
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        # Step 5: avoid exploding gradients
    ...
    # average training perplexity for this epoch
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        train_ppl = train()
        write_log("train", epoch, train_ppl)
        val_loss = evaluate(val_data)
        val_ppl = math.exp(val_loss)
        write_log("valid", epoch, val_ppl)
        print('-' * 89)
